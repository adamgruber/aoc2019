...
describe('Nanofactory', () => {
    [
        ...
        it('should calculate amount of ORE to produce 1 FUEL', () => {
            ...
            const { FUEL } = factory;
            const oreReactions = [];
            for (let i = 1; i < FUEL.length; i += 1) {
                const [qtyNeeded, chemical] = FUEL[i].split(' ');
                const reaction = factory[chemical];
                const isOreReaction = reaction.some(p => `${p}`.match(/ORE/));
                console.log(qtyNeeded, chemical, reaction, isOreReaction);

                if (isOreReaction) {
                    oreReactions.push([qtyNeeded, chemical]);
                }
            }

            console.log(oreReactions);
            // console.log(factory.FUEL);
            // console.log(factory);
            // expect(factory.getOreToFuelRatio()).toEqual(ore);
        });
    });
